...
local SetProps, SetChildren, InsertTheme, Create do
  ...
  local success, debug = pcall(Save, "MBM_HUB.lua")
...
function redzlib:SetTheme(NewTheme)
  ...
  SaveJson("MBM_HUB.lua", redzlib.Save)
...
function redzlib:SetFR()

  local R = tonumber(_G.R)
  local G = tonumber(_G.G)
  local B = tonumber(_G.B)
  redzlib.Save.Color.FR = Color3.fromRGB(R, G, B)
  SaveJson("MBM_HUB.lua", redzlib.Save)
  
  table.foreach(redzlib.Instances, function(_,Val)
    if Val.Type == "Frame" then
      Val.Instance.BackgroundColor3 = Color3.fromRGB(R, G, B)
    end
  end)
end

function redzlib:Setxt()

  local R = tonumber(_G.R)
  local G = tonumber(_G.G)
  local B = tonumber(_G.B)
  redzlib.Save.Color.xt = Color3.fromRGB(R, G, B)
  SaveJson("MBM_HUB.lua", redzlib.Save)
  
  table.foreach(redzlib.Instances, function(_,Val)
    if Val.Type == "Text" then
      Val.Instance[GetColor(Val.Instance)] = Color3.fromRGB(R, G, B)
    end
  end)
end

function redzlib:Settxt()

  local R = tonumber(_G.R)
  local G = tonumber(_G.G)
  local B = tonumber(_G.B)
  redzlib.Save.Color.txt = Color3.fromRGB(R, G, B)
  SaveJson("MBM_HUB.lua", redzlib.Save)
  
  table.foreach(redzlib.Instances, function(_,Val)
    if Val.Type == "DarkText" then
      Val.Instance[GetColor(Val.Instance)] = Color3.fromRGB(R, G, B)
    end
  end)
end

function redzlib:Setst()

  local R = tonumber(_G.R)
  local G = tonumber(_G.G)
  local B = tonumber(_G.B)
  redzlib.Save.Color.st = Color3.fromRGB(R, G, B)
  SaveJson("MBM_HUB.lua", redzlib.Save)
...
function redzlib:Setsb()

  local R = tonumber(_G.R)
  local G = tonumber(_G.G)
  local B = tonumber(_G.B)
  redzlib.Save.Color.sb = Color3.fromRGB(R, G, B)
  SaveJson("MBM_HUB.lua", redzlib.Save)
  
  table.foreach(redzlib.Instances, function(_,Val)
    if Val.Type == "ScrollBar" then
      Val.Instance[GetColor(Val.Instance)] = Color3.fromRGB(R, G, B)
    end
  end)
end

function redzlib:Setth()

  local R = tonumber(_G.R)
  local G = tonumber(_G.G)
  local B = tonumber(_G.B)
  redzlib.Save.Color.th = Color3.fromRGB(R, G, B)
  SaveJson("MBM_HUB.lua", redzlib.Save)
  
  table.foreach(redzlib.Instances, function(_,Val)
    if Val.Type == "Theme" then
      Val.Instance[GetColor(Val.Instance)] = Color3.fromRGB(R, G, B)
    end
  end)
end

function redzlib:Setre()
  ...
  table.foreach(redzlib.Instances, function(_,Val)
    ...
    elseif Val.Type == "Frame" then
      Val.Instance.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    elseif Val.Type == "Stroke" then
      Val.Instance[GetColor(Val.Instance)] = Color3.fromRGB(40, 40, 40)
    elseif Val.Type == "Theme" then
      Val.Instance[GetColor(Val.Instance)] = Color3.fromRGB(88, 101, 242)
    elseif Val.Type == "Text" then
      Val.Instance[GetColor(Val.Instance)] = Color3.fromRGB(243, 243, 243)
    elseif Val.Type == "DarkText" then
      Val.Instance[GetColor(Val.Instance)] = Color3.fromRGB(180, 180, 180)
    elseif Val.Type == "ScrollBar" then
      Val.Instance[GetColor(Val.Instance)] = Color3.fromRGB(88, 101, 242)
    end
  end)
...
function redzlib:MakeWindow(Configs)
  local WTitle = Configs[1] or Configs.Name or Configs.Title or "MBM HUB"
  local WMiniText = Configs[2] or Configs.SubTitle or "by : player.81"
  ...
  function Window:MakeTab(paste, Configs)
    ...
    function Tab:AddSlider(Configs)
      ...
      SliderIcon:GetPropertyChangedSignal("Position"):Connect(function()
        ...
        local NewValue = math.floor((((SliderPos * Max) / Max) * (Max - Min) + Min) * 10 + 0.5 ) / 10
    ...
    function Tab:Addprofile(Configs)
      local Players = game:GetService("Players")
      local player = Players.LocalPlayer
      local code = game.LocalizationService.RobloxLocaleId
      local namesss = player.name
      local userId = player.UserId
      local thumbType = Enum.ThumbnailType.AvatarBust
      local thumbSize = Enum.ThumbnailSize.Size420x420
      local content = Players:GetUserThumbnailAsync(userId, thumbType, thumbSize)
      local dmn = Configs.Description or Configs.Desc or "Player's country : "..code
      
      local proHolder = Create("Frame", Container, {
        Size = UDim2.new(1, 0, 0, 80),
        Name = "Option",
        BackgroundTransparency = 1
      })
      
      local proLabel = Create("TextLabel", InviteHolder, {
        Size = UDim2.new(1, 0, 0, 15),
        Position = UDim2.new(0, 5),
        TextColor3 = Color3.fromRGB(40, 150, 255),
        Font = Enum.Font.GothamBold,
        ...
        BackgroundTransparency = 1,
        TextSize = 10,
        Text = "https://www.roblox.com/users/" .. player.UserId
      })
      
      local FrHolder = InsertTheme(Create("Frame", proHolder, {
        Size = UDim2.new(1, 0, 0, 65),
        AnchorPoint = Vector2.new(0, 1),
        Position = UDim2.new(0, 0, 1),
        BackgroundColor3 = Theme["Color Hub 2"]
      }), "Frame")Make("Corner", FrHolder)
      
      local ImagegLabel = Create("ImageLabel", FrHolder, {
        Size = UDim2.new(0, 30, 0, 30),
        Position = UDim2.new(0, 7, 0, 7),
        Image = content,
        BackgroundTransparency = 1
      })Make("Corner", ImagegLabel, UDim.new(0, 4))Make("Stroke", ImagegLabel)
      
      local pdTitle = InsertTheme(Create("TextLabel", FrHolder, {
        Size = UDim2.new(1, -52, 0, 15),
        Position = UDim2.new(0, 44, 0, 7),
        Font = Enum.Font.GothamBold,
        TextColor3 = Theme["Color Text"],
      ...
      local pdDesc = InsertTheme(Create("TextLabel", FrHolder, {
        Size = UDim2.new(1, -52, 0, 0),
        Position = UDim2.new(0, 44, 0, 22),
        TextWrapped = "Y",
        AutomaticSize = "Y",
      ...
      local cpButton = Create("TextButton", FrHolder, {
        Size = UDim2.new(1, -14, 0, 16),
        AnchorPoint = Vector2.new(0.5, 1),
        Position = UDim2.new(0.5, 0, 1, -7),
        ...
        TextColor3 = Color3.fromRGB(220, 220, 220),
        BackgroundColor3 = Color3.fromRGB(50, 150, 50)
      })Make("Corner", cpButton, UDim.new(0, 5))
      
      local ClicksDelay
      cpButton.Activated:Connect(function()
        setclipboard("https://www.roblox.com/users/" .. player.UserId)
        if ClicksDelay then return end
        
        ClicksDelay = true
        SetProps(cpButton, {
          Text = "Copied to Clipboard",
          BackgroundColor3 = Color3.fromRGB(223, 229, 237),
      ...
      function profile:Visible(Bool)
        if Bool == nil then proHolder.Visible = not proHolder.Visible return end
        proHolder.Visible = Bool
      end
      return profile
    end
    return Tab
